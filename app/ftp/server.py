...
import unicodedata # <--- Importante para normalizaÃ§Ã£o de nomes
...
__all__ = (
    "Permission", "User", "AbstractUserManager", "SQLServerUserManager",
...
class User:
    def __init__(
        ...
        self.base_path = PurePosixPath("/")
        self.home_path = PurePosixPath(f"/{login}")
        self.permissions = [Permission(f"/{login}", readable=True, writable=True)] + (permissions or [])
    ...
    def update(self, d):
        ...
        self.password_algorithm = d.password_algorithm or self.password_algorithm
        ...
        return self
    @classmethod
    def from_record(cls, record: UserRecord):
        permissions = [
            Permission(
                permission.virtual_path.strip(),
                readable=permission.readable,
                writable=permission.writable,
            )
            for permission in record.permissions
            if permission.virtual_path != f"/{record.login}"
        ]
        return cls(
            record.login,
...
class SQLServerUserManager(AbstractUserManager):
    ...
    async def get_user(self, login):
        ...
        user = User.from_record(record) if record and record.enabled else None
        if user is not None:
    ...
    async def authenticate(self, user, password):
        if user.password_algorithm == "bcrypt":
            valid = PasswordManager.verify_password_bcrypt(
                password,
                user.password_hash,
            )
        elif user.password_algorithm == "pbkdf2":
            try:
                stored_hash, stored_salt = user.password_hash.split("$", 1)
            except ValueError:
                return False
            valid = PasswordManager.verify_password_pbkdf2(
                password,
                stored_hash,
                stored_salt,
            )
        ...
        if valid and PasswordManager.needs_rehash(
            user.password_hash,
            user.password_algorithm,
        ):
            new_hash = PasswordManager.hash_password_bcrypt(password)
            await self.repository.update_password(user.user_id, new_hash)
...
class ConnectionConditions:
    ...
    def __call__(self, f):
        ...
        async def wrapper(cls, connection, rest, *args):
            ...
            aggregate = gather(*futures); timeout = 15 if self.wait else 0
...
class Server:
    def __init__(
        ...
        *,
        ...
        self.commands_mapping = {
            "abor": self.abor, "appe": self.appe, "auth": self.auth,
            "cdup": self.cdup, "cwd": self.cwd,
            ...
            "user": self.user, "feat": self.feat, "noop": self.noop,
            "opts": self.opts, "size": self.size, "mdtm": self.mdtm,
        }
    ...
    # âœ… CORREÃ‡ÃƒO: ForÃ§a Encoding UTF-8 na SAÃDA
    async def write_line(self, stream, line):
        try:
            encoded = (line + "\r\n").encode("utf-8")
        except UnicodeEncodeError:
            # Fallback: SanitizaÃ§Ã£o
    ...
    # âœ… CORREÃ‡ÃƒO: ForÃ§a Decoding UTF-8 na ENTRADA
    async def parse_command(self, stream):
        line = await stream.readline()
        if not line: raise ConnectionResetError

        # Tenta decodificar UTF-8 (PadrÃ£o Rclone/RaiDrive)
        try:
        ...
        # NormalizaÃ§Ã£o Unicode (Crucial para acentos)
    ...
    async def dispatcher(self, reader, writer):
        ...
        conn = Connection(
            ...
            control_tls=False, data_protection=False,
        )
                    ...
                    if isinstance(res, bool):
                        ...
                        if f:
                            if cmd == "auth":
                                pending.add(
                                    create_task(
                                        self.auth_then_parse(conn, rest, stream)
                                    )
                                )
                            else:
                            ...
                            pending.add(create_task(self.parse_command(stream)))
                            conn.response("502", "not implemented")
    ...
    async def user(self, conn, rest):
        if self.tls_required and not conn.control_tls:
            conn.response("534", "TLS required before authentication")
            return True
    ...
    async def auth(self, conn, rest):
        if rest.upper() != "TLS":
            conn.response("504", "AUTH type not supported")
            return True
        if self.tls_context is None:
            conn.response("502", "TLS unavailable")
            return True
        if conn.control_tls:
            conn.response("503", "TLS already active")
            return True
        conn.response("234", "AUTH TLS successful")
        await conn.response_queue.join()
        await conn.command_connection.start_tls(self.tls_context)
        conn.control_tls = True
        return True

    async def auth_then_parse(self, conn, rest, stream):
        await self.auth(conn, rest)
        return await self.parse_command(stream)
    ...
    async def list(self, conn, rest):
        ...
        requested_path = "" if rest.lstrip().startswith("-") else rest
        real, virt = self.get_paths(conn, requested_path)
    ...
    async def mlst(self, conn, rest):
        ...
        facts = await self.build_mlsx_string(conn, real)
        conn.response("250", ["Listing", facts, "End"], True)
        return True

    async def build_mlsx_string(self, conn, path):
        stats = await conn.path_io.stat(path)
        item_type = "dir" if stats.st_mode & 0x4000 else "file"
        modified = strftime("%Y%m%d%H%M%S", gmtime(stats.st_mtime))
        return (
            f"Type={item_type};Size={stats.st_size};"
            f"Modify={modified}; {path.name}"
        )
    ...
    async def pbsz(self, c, r):
        if not c.control_tls:
            c.response("503", "PBSZ requires TLS")
        elif r.strip() != "0":
            c.response("501", "PBSZ must be zero")
        else:
            c.response("200", "PBSZ=0")
        return True

    async def prot(self, c, r):
        protection = r.strip().upper()
        if not c.control_tls:
            c.response("503", "PROT requires TLS")
        elif protection == "P":
            c.data_protection = True
            c.response("200", "Data channel protected")
        elif protection == "C" and not self.tls_required:
            c.data_protection = False
            c.response("200", "Clear data channel")
        else:
            c.response("536", "Only PROT P is permitted")
        return True
    ...
    async def _pasv_common(self, conn, epsv):
        ...
        if not conn.future.passive_server.done():
            if self.tls_required and not conn.data_protection:
                conn.response("536", "PROT P required")
                return False
            ports = self.passive_ports or (0,)
            for passive_port in ports:
                try:
                    conn.passive_server = await start_server(
                        h,
                        conn.server_host,
                        passive_port,
                        ssl=self.tls_context if conn.data_protection else None,
    ...
    async def feat(self, c, r):
        ...
        c.response("211", features, True)
        return True

    async def noop(self, c, r):
        c.response("200", "NOOP")
        return True

    async def opts(self, c, r):
        if r.strip().upper() == "UTF8 ON":
            c.response("200", "UTF8 enabled")
        else:
            c.response("501", "Unsupported option")
        return True

    @ConnectionConditions(ConnectionConditions.login_required)
    @PathConditions(PathConditions.path_must_exists, PathConditions.path_must_be_file)
    async def size(self, c, r):
        real, _ = self.get_paths(c, r)
        stats = await c.path_io.stat(real)
        c.response("213", str(stats.st_size))
        return True
    ...
    async def mdtm(self, c, r):
        real, _ = self.get_paths(c, r)
        stats = await c.path_io.stat(real)
        c.response("213", strftime("%Y%m%d%H%M%S", gmtime(stats.st_mtime)))
        return True
    ...
    async def mlsd(self, c, r):
        ...
        async def mlsd_worker(self, conn, rest):
            stream = conn.data_connection
            del conn.data_connection
            async with stream:
                async for path in conn.path_io.list(real):
                    facts = await self.build_mlsx_string(conn, path)
                    await stream.write((facts + "\r\n").encode("utf-8"))
            conn.response("226", "MLSD complete")
            return True

        real, _ = self.get_paths(c, r)
        task = create_task(mlsd_worker(self, c, r))
        c.extra_workers.add(task)
        c.response("150", "MLSD starting")
        return True
    ...
    async def nlst(self, c, r):
        ...
        @worker
        async def nlst_worker(self, conn, rest):
            stream = conn.data_connection
            del conn.data_connection
            async with stream:
                async for path in conn.path_io.list(real):
                    await stream.write((path.name + "\r\n").encode("utf-8"))
            conn.response("226", "NLST complete")
            return True

        real, _ = self.get_paths(c, r)
        task = create_task(nlst_worker(self, c, r))
        c.extra_workers.add(task)
        c.response("150", "NLST starting")
        return True
