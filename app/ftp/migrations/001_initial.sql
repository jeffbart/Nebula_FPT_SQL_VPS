...
BEGIN
    CREATE TABLE nebula.users (
        ...
        CONSTRAINT CK_users_password_algorithm
            CHECK (password_algorithm IN ('bcrypt', 'pbkdf2'))
    );
END;
...
BEGIN
    CREATE TABLE nebula.permissions (
        ...
        CONSTRAINT FK_permissions_users
            FOREIGN KEY (user_id) REFERENCES nebula.users(user_id) ON DELETE CASCADE,
...
BEGIN
    CREATE TABLE nebula.nodes (
        ...
        CONSTRAINT CK_nodes_status
            CHECK (status IN ('staging', 'uploading', 'completed', 'failed', 'deleting'))
    );
...
BEGIN
    CREATE TABLE nebula.file_parts (
        ...
        CONSTRAINT FK_file_parts_nodes
            FOREIGN KEY (node_id) REFERENCES nebula.nodes(node_id) ON DELETE CASCADE,
...
BEGIN
    CREATE TABLE nebula.operation_history (
        ...
        occurred_at datetime2(3) NOT NULL
            CONSTRAINT DF_operation_history_occurred_at DEFAULT SYSUTCDATETIME()
    );
